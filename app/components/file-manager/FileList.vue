...
import { Folder, File, ChevronRight, Link, Image, Move, Trash2, Edit, CheckSquare, Square } from 'lucide-vue-next'
  ...
  <Card>
    <ScrollArea class="h-[calc(100vh-220px)]">
      <div class="p-4">
        ...
        <div v-else-if="!props.folders.length && !props.files.length" class="text-center py-8 text-muted-foreground">
        ...
        <div v-else class="space-y-1">
          ...
          >
            ...
            >
              <CheckSquare v-if="props.selectedFiles.has(file.pathname)" class="h-5 w-5 text-primary" />
              <Square v-else class="h-5 w-5 text-muted-foreground" />
            </button>
            ...
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button variant="ghost" size="icon" class="h-8 w-8" @click="emit('copy-url', { pathname: file.pathname, type: 'raw' })">
                <Link class="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" class="h-8 w-8" @click="emit('copy-url', { pathname: file.pathname, type: 'markdown' })">
                <Image class="h-4 w-4" />
              </Button>
              ...
              <Button variant="ghost" size="icon" class="h-8 w-8 text-destructive" @click="emit('delete', file.pathname)">
